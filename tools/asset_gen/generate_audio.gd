...
func _sfx() -> void:
	...
	b = _buf(0.2)                      # raptor bite snap
	_burst(b, 0.0, 0.05, 0.9, 0.8, 0.4)
	_burst(b, 0.07, 0.06, 0.7, 0.8, 0.4)
	_tone(b, 0.0, 0.12, 300, 190, 0.4)
	_save("bite", b)

	b = _buf(0.55)                     # aggro growl
	_tone(b, 0.0, 0.55, 95, 62, 0.7, 2, 0.25)
	_burst(b, 0.0, 0.5, 0.25, 0.15, 0.08)
	_save("growl", b)

	b = _buf(0.32)                     # raptor pain screech
	_tone(b, 0.0, 0.3, 1050, 560, 0.55, 1, 0.35)
	_burst(b, 0.0, 0.2, 0.2, 0.7, 0.3)
	_save("raptor_hurt", b)

	b = _buf(0.95)                     # raptor death screech
	_tone(b, 0.0, 0.85, 1250, 190, 0.6, 1, 0.5)
	_tone(b, 0.1, 0.7, 620, 120, 0.35, 2, 0.3)
	_burst(b, 0.4, 0.5, 0.35, 0.4, 0.08)
	_save("raptor_die", b)

	b = _buf(0.26)                     # caveman grunt
	_tone(b, 0.0, 0.22, 220, 120, 0.7, 1, 0.15)
	_burst(b, 0.0, 0.15, 0.3, 0.4, 0.15)
	_save("player_hurt", b)
	...
	b = _buf(0.3)                      # jaws chomp
	_burst(b, 0.0, 0.07, 1.0, 0.9, 0.4)
	_tone(b, 0.0, 0.2, 80, 45, 0.8)
	_burst(b, 0.1, 0.08, 0.9, 0.9, 0.4)
	_save("chomp", b)
